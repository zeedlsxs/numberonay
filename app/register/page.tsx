...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        setToast({ type: "error", message: data.error || "Kayıt tamamlanamadı." })
        return
      }
      await refresh()
      setToast({ type: "success", message: "Kayıt başarılı! Panele yönlendiriliyorsunuz..." })
    ...
    <div className="min-h-screen bg-[#F8FAFC] flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      ...
      <Card className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-[#171725]">Kayıt Ol</h1>
          <p className="text-[#6B7280] mt-2">Yeni hesabınızı oluşturun</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex items-start gap-3">
            <input
              ...
              className="mt-1 w-4 h-4 rounded border-[#E5E7EB] text-[#2196F3] focus:ring-[#2196F3]"
            />
            <label htmlFor="terms" className="text-sm text-[#6B7280]">
              <Link href="/terms" className="text-[#2196F3] hover:underline">
                Kullanım Şartları
              </Link>{" "}
              ve{" "}
          ...
          {errors.acceptTerms && (
            <p className="text-sm text-[#EF4444]">{errors.acceptTerms}</p>
          )}
        ...
        <div className="mt-6">
          ...
          >
            ...
            {siteConfig.googleAuthEnabled ? "Google ile Kayıt Ol" : "Google ile kayıt özelliği yapılandırılmamış"}
          </Button>
        </div>
